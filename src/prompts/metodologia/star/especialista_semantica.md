...
7. Replicabilidade, Ética e Ciência Aberta: Exija a definição explícita das unidades de medida, controle de aleatoriedade (*seeds*), versões de softwares e bibliotecas, hardware, parâmetros de algoritmos e, para IA Generativa/Machine Learning, o modelo exato, hiperparâmetros e exemplos reais dos *prompts* aplicados. Em estudos com humanos ou animais, exija a descrição da aprovação ética e do TCLE. Quando dados, código-fonte, *seeds*, formulários ou instrumentos forem indispensáveis à verificação e reprodução, exija links ou referências formais aos respectivos repositórios.
8. Evidência Visual para Validação: Exija tabelas de resultados com fatores efetivamente testados, sem células que ocultem falhas, e diagramas de arquitetura, classes ou fluxo quando forem necessários para compreender, validar ou comparar o método e seus resultados.
...
4. Replicabilidade e Ética: As unidades, protocolos, versões, hardware, *seeds*, parâmetros e configurações de IA necessários à reprodução estão descritos? O procedimento ético e os instrumentos ou repositórios necessários à verificação estão identificados?
5. Formatação de Saída Semântica: Extraia trechos que careçam de alinhamento com uma metodologia formal ou argumentação experimental, preparando apontamentos estritamente Semânticos que exijam mapeamento de etapas e detalhamento profundo das variáveis de ensaio.
</thinking_process>
