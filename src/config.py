...
class Settings(BaseModel):
    ...
    # Model Defaults
    embedding_model: str = "openai/text-embedding-v4" # Modelo de embedding via Dashscope
    embedding_api_base: str = "https://dashscope-intl.aliyuncs.com/compatible-mode/v1"
    embedding_cost_per_million_tokens: float = 0.07


    #router_model: str = "gemini/gemma-3-27b-it"
    router_model: str = "openrouter/deepseek-v4-flash"
    evaluator_model: str = "openrouter/deepseek-v4-flash"
    synthesizer_model: str = "openrouter/deepseek-v4-flash"
    synthesizer_max_tokens: int = 65536
    ...
    # ChromaDB Settings
    chroma_db_dir: str = Field(default_factory=lambda: os.getenv("CHROMA_DB_DIR", "./chroma_db_v4"))
    ...
    available_models: Dict[str, str] = {
        ...
        "openai/gpt-5.5": "Custo ALTO. (Input: \$5.00/1M, Output: \$30.00/1M tokens). Modelo de fronteira da OpenAI com o mais alto nível de inteligência. Excelente para raciocínio lógico profundo e tarefas que demandam máxima precisão e qualidade.",
        ...
        #"dashscope/deepseek-v4-flash-us": "Custo BAIXO. (Input: \$0.20/1M, Output: \$0.40/1M tokens). Modelo DeepSeek V4 Flash US de alto desempenho.",
        
        "openrouter/deepseek-v4-flash": "Custo BAIXO. (Input: \$0.09/1M, Output: \$0.18/1M tokens). Modelo DeepSeek V4 Flash via OpenRouter."
    }
    ...
    def validate_prompts_structure(self):
        ...
        for section in os.listdir(prompts_dir):
            ...
            if not os.path.isdir(section_path):
                continue
            if not any(files for _, _, files in os.walk(section_path)):
                continue
